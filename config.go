...
func ParseFromIni(path string) (*Config, error) {
	...
	for k, v := range dict {
		if strings.HasPrefix(k, "topic_") {
			...
			var compressed bool
			if !present || c == "n" || c == "N" || c == "0" || c == "f" || c == "F" || c == "false" {
				compressed = false
			} else if c == "y" || c == "Y" || c == "1" || c == "t" || c == "T" || c == "true" {
				compressed = true
			} else {
				return nil, errors.New("Invalid config format, invalid compressed value " + c)
			}
			...
			if !present {
				...
				if err != nil {
					return nil, errors.New("Invalid config format, invalid timeout value " + timeoutStr)
				}
			}
			...
			logTopic := &LogTopic{topic, logBasePath, logFilePattern, encoding, compressed, timeout, checkpointName};
